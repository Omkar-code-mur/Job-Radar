...
api.MapPatch("/sources/{id}", async (HttpContext context, string id, SourceUpdate input, CancellationToken ct) => { var user = await userIdentityStore.GetOrCreateAsync(context.User, adminEmail, ct); if (user?.Role != "ADMIN") return Results.Forbid(); var source = await store.UpdateSourceAsync(id, input, ct); return source is null ? Results.NotFound(new { error = "Source not found." }) : Results.Ok(source); });
