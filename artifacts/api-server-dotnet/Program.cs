...
builder.Logging.AddSimpleConsole(options => { options.TimestampFormat = "yyyy-MM-dd HH:mm:ss "; options.SingleLine = true; });
...
app.Use(async (context, next) =>
{
    ...
    using (app.Logger.BeginScope(new Dictionary<string, object> { ["RequestId"] = requestId, ["Method"] = context.Request.Method, ["Path"] = context.Request.Path.Value ?? string.Empty }))
    {
        var stopwatch = Stopwatch.StartNew();
        app.Logger.LogInformation("API request started {Method} {Path}", context.Request.Method, context.Request.Path);
        try { await next(context); app.Logger.LogInformation("API request completed {Method} {Path} with {StatusCode} in {ElapsedMs} ms", context.Request.Method, context.Request.Path, context.Response.StatusCode, stopwatch.ElapsedMilliseconds); }
        catch (OperationCanceledException) when (context.RequestAborted.IsCancellationRequested) { app.Logger.LogWarning("API request cancelled {Method} {Path} after {ElapsedMs} ms", context.Request.Method, context.Request.Path, stopwatch.ElapsedMilliseconds); throw; }
        catch (Exception exception) { app.Logger.LogError(exception, "API request failed {Method} {Path} after {ElapsedMs} ms", context.Request.Method, context.Request.Path, stopwatch.ElapsedMilliseconds); throw; }
    }
});
...
api.MapPost("/sources", async (HttpContext context, SourceInput input, CancellationToken ct) => { var user = await userIdentityStore.GetOrCreateAsync(context.User, adminEmail, ct); if (user?.Role != "ADMIN") return Results.Forbid(); var source = await store.AddSourceAsync(input, ct); return source is null ? Results.BadRequest(new { error = "Company, source name, type, and URL are required." }) : Results.Created($"/api/sources/{source.Id}", source); });
api.MapPatch("/sources/{id}", async (HttpContext context, string id, SourceUpdate input, CancellationToken ct) => { var user = await userIdentityStore.GetOrCreateAsync(context.User, adminEmail, ct); if (user?.Role != "ADMIN") return Results.Forbid(); try { var source = await store.UpdateSourceAsync(id, input, ct); return source is null ? Results.NotFound(new { error = "Source not found." }) : Results.Ok(source); } catch (SourceConfigurationException exception) { return Results.BadRequest(new { error = exception.Message }); } });
api.MapDelete("/sources/{id}", async (HttpContext context, string id, CancellationToken ct) => { var user = await userIdentityStore.GetOrCreateAsync(context.User, adminEmail, ct); if (user?.Role != "ADMIN") return Results.Forbid(); return await store.DeleteSourceAsync(id, ct) ? Results.NoContent() : Results.NotFound(new { error = "Source not found." }); });
api.MapPost("/sources/{id}/scan", async (string id, HttpContext context, JobSourceFetcherFactory sourceFetcherFactory, CancellationToken ct) => { var user = await userIdentityStore.GetOrCreateAsync(context.User, adminEmail, ct); if (user?.Role != "ADMIN") return Results.Forbid(); try { return Results.Ok(await store.ScanAsync(user.Id, [id], sourceFetcherFactory, ct)); } catch (ScanRequestException exception) { return Results.Json(new { error = exception.Message }, statusCode: exception.StatusCode); } });
...
api.MapPost("/profile/import/preview", async (HttpContext context, JsonElement input, CancellationToken ct) =>
{
    var user = await userIdentityStore.GetOrCreateAsync(context.User, adminEmail, ct);
    if (user is null) return Results.Unauthorized();
    if (input.ValueKind != JsonValueKind.Object) return Results.BadRequest(new { error = "Paste the JSON object returned by your AI assistant." });
    try
    {
        var existing = await store.GetProfileAsync(user.Id, ct);
        var mapped = ProfileImportService.ToProfileInput(input, existing);
        return Results.Ok(new { profile = mapped, details = input });
    }
    catch (Exception ex) when (ex is JsonException || ex is InvalidOperationException)
    {
        return Results.BadRequest(new { error = "The profile JSON could not be parsed." });
    }
});
api.MapPost("/profile/import/confirm", async (HttpContext context, JsonElement input, CancellationToken ct) =>
{
    var user = await userIdentityStore.GetOrCreateAsync(context.User, adminEmail, ct);
    if (user is null) return Results.Unauthorized();
    if (input.ValueKind != JsonValueKind.Object) return Results.BadRequest(new { error = "A valid imported profile is required." });
    var existing = await store.GetProfileAsync(user.Id, ct);
    var mapped = ProfileImportService.ToProfileInput(input, existing);
    var saved = await store.SaveProfileAsync(user.Id, mapped, ct);
    await profileImportService.SaveDetailsAsync(user.Id, input, ct);
    return Results.Ok(new { profile = saved, imported = true });
});
...
api.MapPost("/dream-companies/{companyId}", async (HttpContext context, string companyId, CancellationToken ct) => { var user = await userIdentityStore.GetOrCreateAsync(context.User, adminEmail, ct); if (user is null) return Results.Unauthorized(); var sources = await store.GetSourcesAsync(ct); if (!sources.Any(source => source.CompanyId == companyId && IsMonitorableSource(source))) return Results.BadRequest(new { error = "This company is not currently monitorable by Job Radar." }); await workspaceStore.AddDreamCompanyAsync(user.Id, companyId, ct); return Results.NoContent(); });
api.MapDelete("/dream-companies/{companyId}", async (HttpContext context, string companyId, CancellationToken ct) => { var user = await userIdentityStore.GetOrCreateAsync(context.User, adminEmail, ct); if (user is null) return Results.Unauthorized(); await workspaceStore.RemoveDreamCompanyAsync(user.Id, companyId, ct); return Results.NoContent(); });
api.MapGet("/workspace/settings", async (HttpContext context, CancellationToken ct) => { var user = await userIdentityStore.GetOrCreateAsync(context.User, adminEmail, ct); if (user is null) return Results.Unauthorized(); return Results.Ok(await workspaceStore.GetWorkspaceSettingsAsync(ct)); });
api.MapPut("/workspace/settings", async (HttpContext context, WorkspaceSettings input, CancellationToken ct) => { var user = await userIdentityStore.GetOrCreateAsync(context.User, adminEmail, ct); if (user?.Role != "ADMIN") return Results.Forbid(); await workspaceStore.SaveWorkspaceSettingsAsync(input, ct); return Results.Ok(input); });
...
public record Job(string Id, string CompanyId, string SourceId, string ExternalJobId, string Company, string Title, string Description, string Location, string WorkplaceType, string Department, string EmploymentType, string PostedDate, string FirstSeenAt, string LastSeenAt, string ApplicationUrl, string SourceUrl, int Score, bool IsMatch, bool Notified, string[] MatchedSkills, string[] MissingSkills, Breakdown Breakdown);
...
public record SourceScanFailure(string SourceId, string SourceType, string Error, int MalformedRecordCount, string[] Diagnostics);
public record ScanResult(int SourcesScanned, int JobsFetched, int NewJobs, int MatchedJobs, int NotificationsSent, IReadOnlyList<SourceScanFailure> Failures);
public sealed class ScanRequestException(int statusCode, string message) : Exception(message)
{
