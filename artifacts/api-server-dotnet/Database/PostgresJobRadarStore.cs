...
public sealed class PostgresJobRadarStore
{
    ...
    public async Task<ScanResult> ScanAsync(IReadOnlyCollection<string> ids, GreenhouseJobSource greenhouse, CancellationToken cancellationToken = default) { var sources=(await GetSourcesAsync(cancellationToken)).Where(item=>ids.Contains(item.Id)&&item.Enabled&&item.Type=="GREENHOUSE_API").ToList(); var fetched=0; var added=0; foreach(var source in sources) { var jobs=await greenhouse.FetchAsync(source,source.CompanyName,cancellationToken); var existing=(await GetJobsAsync(null,null,null,null,cancellationToken)).Select(item=>item.Id).ToHashSet(); await UpsertJobsAsync(jobs,cancellationToken); fetched+=jobs.Count; added+=jobs.Count(item=>!existing.Contains(item.Id)); await UpdateSourceHealthAsync(source.Id,jobs.Count,null,cancellationToken); } return new(sources.Count,fetched,added,added,0); }
